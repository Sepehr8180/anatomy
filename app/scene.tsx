...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  const up=(e:PointerEvent)=>{
   ...
   let nearest=Infinity,found=-1;const current=latest.current;const hasSolid=atlas.parts.some((p,i)=>!current.hidden.includes(p.id)&&p.system!=='integumentary'&&data[i*4+3]>.5);
   pickers.forEach((mesh,i)=>{if(!mesh||current.hidden.includes(atlas.parts[i].id)||data[i*4+3]<.5||(hasSolid&&atlas.parts[i].system==='integumentary'))return;worldBox.copy(bounds[i]).translate(mesh.position);if(!raycaster.ray.intersectBox(worldBox,hitPoint))return;const hits=raycaster.intersectObject(mesh,false);if(hits[0]&&hits[0].distance<nearest){nearest=hits[0].distance;found=i;}});
  ...
  const animate=()=>{
   ...
   if(changed||moving||lastExtent<0){
    ...
    atlas.parts.forEach((p,i)=>{
     ...
     const selected=selection.has(p.id),isHidden=hidden.has(p.id);data.set([dx,dy,dz,(!isHidden&&(s.isolate?selected:visible.has(p.system)||selected))?1:0],i*4);selectedData[i*4]=selected?255:0;
   ...
   if(dirty){renderer.render(scene,camera);targets=[];if(amount>.45){const current=latest.current;const hasSolid=atlas.parts.some((p,i)=>!current.hidden.includes(p.id)&&p.system!=='integumentary'&&data[i*4+3]>.5);atlas.parts.forEach((p,i)=>{if(s.hidden.includes(p.id)||data[i*4+3]<.5||(hasSolid&&p.system==='integumentary'))return;let left=Infinity,right=-Infinity,top=Infinity,bottom=-Infinity;for(let corner=0;corner<8;corner++){projected.set(p.bounds[(corner&1)?1:0][0]+data[i*4],p.bounds[(corner&2)?1:0][1]+data[i*4+1],p.bounds[(corner&4)?1:0][2]+data[i*4+2]).project(camera);const x=(projected.x+1)*el.clientWidth/2,y=(1-projected.y)*el.clientHeight/2;left=Math.min(left,x);right=Math.max(right,x);top=Math.min(top,y);bottom=Math.max(bottom,y);}projected.copy(centers[i]).add(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])).project(camera);if(projected.z< -1||projected.z>1)return;targets.push({index:i,x:(projected.x+1)*el.clientWidth/2,y:(1-projected.y)*el.clientHeight/2,left,right,top,bottom});});}dirty=false;}
